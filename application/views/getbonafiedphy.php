                                    ...
                                    <div class="row">
                                        <div class="col-md-10">
                                            <p style="font-size: x-large;margin-left: 21%;">
                                                This is to certify that <u><?php echo $studdata[0]['student_name'];?></u> is a bonafide student of this school studying in Senior KG (Year <?php  if(date("m") < 7 ) { echo date("Y",strtotime('-1 year')) ?> - <?php echo substr(date('Y'),-2);  }else { echo date("Y") ?> - <?php echo substr(date('Y', strtotime('+1 year')),-2); } ?>). Child has taken
                                                admission for <?php if($studdata[0]['fk_classId']==1){ echo "Play Group"; }else if($studdata[0]['fk_classId']== 2) { echo "Nursery KG"; }else if($studdata[0]['fk_classId']==3){ echo "Junior KG"; }else if($studdata[0]['fk_classId']==3){ echo "Senior KG"; }else if($studdata[0]['fk_classId']==3){ echo "Grade 1"; }else if($studdata[0]['fk_classId']==3){ echo "Grade 2"; }else if($studdata[0]['fk_classId']==3){ echo "Grade 3"; }else if($studdata[0]['fk_classId']==3){ echo "Grade 4"; }else{ echo "No Class Found";} ?>  in year <?php  if(date("m") < 7 ) { echo date("Y",strtotime('-1 year')) ?> - <?php echo substr(date('Y'),-2);  }else { echo date("Y") ?> - <?php echo substr(date('Y', strtotime('+1 year')),-2); } ?>.</p>
                                                <p style="font-size: x-large;margin-left: 21%;" ><?php if(['gender']=="Male" || ['gender']=="Boy") { echo "His"; }else{ echo "She"; }?> birth date as recorded in the General Register of the school is <?php echo date("d",strtotime($studdata[0]['usr_dob'])); ?> 
                                            ...
                                            <p style="font-size: x-large;margin-left: 21%;">To the best of our knowledge <?php if($studdata[0]['gender']=="Male" || $studdata[0]['gender']=="Boy") { echo "he"; }else{ echo "She"; }?> bears a good moral character.</p>
                                        </div>
                                    </div>
                                    <div class="row">
