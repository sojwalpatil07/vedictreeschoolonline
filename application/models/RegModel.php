...
function filterclasswise_receipt($fk_classId){
    $this->db->trans_start();
    		$this->db->where('grade',$fk_classId);
...
function add_division($data){
    
    $this->db->trans_start();
	$result = $this->db->insert('tbl_add_division',$data);
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	   $this->db->trans_rollback(); 
...
function delete_division($divId){
	...
	$result = $this->db->delete('tbl_add_division');
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	   $this->db->trans_rollback(); 
...
function cechknamedivision($divName){
       $this->db->trans_start();
    $this->db->where('divName', $divName);
	$result = $this->db->get('tbl_add_division')->result_array();
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	   $this->db->trans_rollback(); 
...
function show_pending_physical_nursery($currentdate,$lastdayofmonth,$classId){
	...
	$this->db->where('first_installment_date <=',$lastdayofmonth);
	$this->db->or_where('second_installment_date >=',$currentdate);
	$this->db->where('second_installment_date <=',$lastdayofmonth);
	$this->db->or_where('third_installment_date >=',$currentdate);
	$this->db->where('third_installment_date <=',$lastdayofmonth);
	$this->db->or_where('four_installment_date >=',$currentdate);
	$this->db->where('four_installment_date <=',$lastdayofmonth);
	$this->db->or_where('five_installment_date >=',$currentdate);
	$this->db->where('five_installment_date <=',$lastdayofmonth);
	$this->db->join('studentreg','studentreg.studentMobile = tbl_add_physical_fees.student_mobile','left');
	$res = $this->db->get('tbl_add_physical_fees')->result_array();
...
function show_pending_physical_nursery_withoutclass($currentdate,$lastdayofmonth){
    $this->db->trans_start();
    $this->db->select("*");
	$this->db->where('studentreg.studentStatus',1);
	$this->db->where('first_installment_date >=',$currentdate);
...
function addeventsdata($insertData){
    
    $this->db->trans_start();
	$res = $this->db->insert('tbl_events',$insertData);
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function eventlist(){
    	...
    	$res = $this->db->get('tbl_events')->result_array();
...
function getinsternationallead(){
    
    $res = $this->db->get('tbl_internation_lead')->result_array();
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function international_gallery(){
    $res = $this->db->get('tbl_internation_gallery')->result_array();
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function delete_international_gallery($gid){
    
    $this->db->trans_start();
      	$this->db->where('gid',$gid);
	$res = $this->db->delete('tbl_internation_gallery');
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function international_review(){
    $res = $this->db->get('tbl_internation_review')->result_array();
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function delete_international_review($rid){
    
    $this->db->trans_start();
    $this->db->where('rid',$rid);
	$res = $this->db->delete('tbl_internation_review');
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function add_international_gallery($data){
     $this->db->trans_start();
     $data = array(
                        'gname'   => $data['gname'],
                        'gdate'    => $data['gdate'],
	...
	$res = $this->db->insert('tbl_internation_gallery',$data);
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function add_international_review($data){
     $this->db->trans_start();
     $data = array(
                        'rname'       => $data['rname'],
                        'rdate'       => $data['rdate'],
	...
	$res = $this->db->insert('tbl_internation_review',$data);
	$this->db->trans_complete();
	if ($this->db->trans_status() === FALSE)
	{
	  $this->db->trans_rollback();  
...
function addAttendance($studIds,$techerId,$classId,$div,$date){
    
    $this->db->trans_start();
    $res = 0;
 
    $studIds = explode(",",$studIds);
    
    $studIdss = count($studIds);
   $result = array();
    for($i=0; $i < $studIdss; $i++)
    {
        
                $this->db->select("*");
                $this->db->where('studId',$studIds[$i]);
                $this->db->where('attddate',date("Y-m-d",strtotime($date)));
                $this->db->where('techerId',$techerId);
        $result = $this->db->get('tbl_attendace')->result_array();
     ...
     for($i=0; $i < $studIdss; $i++)
    {
         if(empty($result)){
                     ...
                     $data = array(
                                    ...
                                    'classId'        => $classId,
                                    'divName'        => $div,
                                    'attddate'       => date("Y-m-d",strtotime($date)),
                                    
                                  ); 
                	$res = $this->db->insert('tbl_attendace',$data);
                  $res = 1;
         }else{
                  $res =  0;       
         }
...
function listattd($techerId,$classId,$divName){
    $this->db->select("*");
    $this->db->where('techerId',$techerId);
...
public function getbonafiedphy($id){
     $this->db->trans_start();
